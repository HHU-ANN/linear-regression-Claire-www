...
def lasso(data_input):
    ...
    X_train = np.hstack((np.ones((X_train.shape[0], 1)), X_train))
    # Lasso回归
    def lasso_regression(X, y, alpha, max_iter=1000, tol=1e-4, eta=0.01, decay=0.5):
    ...
    # 默认alpha=0.1
    alpha = 0.1
    prediction = lasso_regression(X_train, y_train, alpha, max_iter=1000, tol=1e-4, eta=0.01, decay=0.5)
    # 预测
